...
def main(greet):
    ...
    while True:
        ...
        Cam=tk.Canvas(cam,width=800,height=242)
        Cam.pack()
        vid = tk.PhotoImage(file=cv2.imshow("Output", img))
        Cam.create_image(0,0,anchor=tk.NW,image=vid)
        Cam.grid(row=50,column=50,pady=20,padx=100)
...
Logo=tk.Canvas(logo,width=800,height=242)
Logo.pack()
...
Logo.grid(row=50,column=50,pady=20,padx=100)
